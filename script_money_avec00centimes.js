...
function ConvNumberLetter_fr(Nombre, bCheckFloat) { 
    ...
    if(bCheckFloat) {
        ...
        {
         if(strNombre.indexOf('.00')==-1)
         {
            strLetter= strLetter + ' (centimes)';
            strLetter = strLetter.replace("(Dhs)", "(dirhams) et ");
         }else
         {
            strLetter= strLetter + ' (centimes)';
            strLetter = strLetter.replace("(Dhs)", "(dirhams) et zero");
         }
        }else
        {
            strLetter= strLetter + ' (centimes)';
...
function ConvNumberLetter(Nombre, Devise, Langue) {
    ...
    if(ConvNumDizaine(byDec, Langue)!="")
    var NumberLetter = ConvNumEnt(parseFloat(dblEnt), Langue) + strDev + " et " + ConvNumDizaine(byDec, Langue) + strCentimes ;
    else
